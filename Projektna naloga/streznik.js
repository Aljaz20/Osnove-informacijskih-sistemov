...
function vrniHtmlZastavico(trenutnaStranka, povratniKlic) {
    ...
    //Brez te implementacije dela za USA, ampak ne za Indijo in Nizozemsko
    vrniDrzavoStranke(trenutnaStranka, (drzava)  => {
        let naslov = 'https://restcountries.com/v3.1/name/' + drzava + '?fields=cca2';
        axios.get(naslov).then(function (response) {
            ...
            let link = "<img src=https://flagcdn.com/16x12/" + kratica.toLowerCase() + ".png width=\"16\" height=\"12\" alt=" + drzava +"></img>";
...
function popust(ime) {
    ...
    for (let i = 0; i < ime.length; i++) {
        let crka = ime.charAt(i);
        if(crka.charCodeAt(0) < 65 || crka.charCodeAt(0) > 90){
            continue;
        }
        if (crke[crka]) {
        crke[crka]++;
    ...
    let najpogostejsa_crka = null;
    let najpogostejsa_count = 0;
    for (let crka in crke) {
        if (crke[crka] > najpogostejsa_count || (crke[crka] == najpogostejsa_count && crka.charCodeAt(0) < najpogostejsa_crka.charCodeAt(0))) {
...
streznik.get("/prijava", (zahteva, odgovor) => {
    vrniStranke((napaka1, stranke) => {
        vrniRacune((napaka2, racuni) => {
            for (let i = 0; stranke != null && i < stranke.length; i++) stranke[i].stRacunov = 0;
            ...
            vrniSeznamDestinacij((napaka, destinacije) => {
                ...
                let hostli = {};
                atrakcije["vsi"] = 0;
                atrakcije["izvenLjubljane"] = 0;
                atrakcije["spletnaStran"] = 0;
                atrakcije["wikidata"] = 0;
                zoo["vsi"] = 0;
                zoo["izvenLjubljane"] = 0;
                zoo["spletnaStran"] = 0;
                zoo["wikidata"] = 0;
                hostli["vsi"] = 0;
                hostli["izvenLjubljane"] = 0;
                hostli["spletnaStran"] = 0;
                hostli["wikidata"] = 0;

                destinacije.forEach((vrstica) => {
                    if (vrstica.vrstaDestinacije == "živalski vrt") {
                        zoo["vsi"]++;
                        if (vrstica.mesto != null && vrstica.mesto != "Ljubljana") zoo["izvenLjubljane"]++;
                        if (vrstica.website != null) zoo["spletnaStran"]++;
                        if (vrstica.wikidata != null) zoo["wikidata"]++;
                    } else if (vrstica.vrstaDestinacije == "atrakcija") {
                        atrakcije["vsi"]++;
                        if (vrstica.mesto != null && vrstica.mesto != "Ljubljana") atrakcije["izvenLjubljane"]++;
                        if (vrstica.website != null) atrakcije["spletnaStran"]++;
                        if (vrstica.wikidata != null) atrakcije["wikidata"]++;
                    } else {
                        hostli["vsi"]++;
                        if (vrstica.mesto != null && vrstica.mesto != "Ljubljana") hostli["izvenLjubljane"]++;
                        if (vrstica.website != null) hostli["spletnaStran"]++;
                        if (vrstica.wikidata != null) hostli["wikidata"]++;
                    }
                });
